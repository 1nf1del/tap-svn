...
static bool  returnFromRecycleWindow;
static bool  fileDeleted;
static bool  fileRestored;
static char  recycleCommandOption;



static byte* recycleBinWindowCopy;
...
{
    // A file is placed in the "recycle bin" by replacing the ".rec" with ".del"
     
	char str[256], str1[256], str2[256], counter[6];
	bool fileAlreadyExists;
	int fileIncrement, result;
	
	// If we try to recycle a file and there is already an existing file in the recycle-bin
	// with the same name, we will append an incrementing counter to the file name.
    ...
    strncpy( str, currentFile.name, 256 );   // Copy the current filename
	StripFileName( str );                    // Strip off the ".rec"
    strncpy( str2, str, 256 );               // Copy the stripped off filename in case we need to rename on a move.
	strcat( str, ".del" );	                 // Replace ".rec" with ".del"

    // Check if the file already exists.
    fileAlreadyExists = TAP_Hdd_Exist(str);
    ...
    while (fileAlreadyExists)
    {
         ...
         strncpy( str, str2, 256);                    // Copy back the original filename
         strcat(  str, counter);                      // Append the counter text to the filename
	     strcat(  str, ".del" );	                  // Append the ".del" to the end.
         fileAlreadyExists = TAP_Hdd_Exist(str);      // Check if the file exists.
    }
