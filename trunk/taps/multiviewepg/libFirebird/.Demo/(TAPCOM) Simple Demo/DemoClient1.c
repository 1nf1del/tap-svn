...
#define PROGRAM_NAME    "TAPCOM DemoClient 1"

TAP_ID                  (0x8E0A42F9);
TAP_PROGRAM_NAME	(PROGRAM_NAME);
TAP_AUTHOR_NAME		("FireBird");
TAP_DESCRIPTION		("TAPCOM DemoClient 1");
  ...
  if (event == EVT_KEY)
  {
    switch (param1)
    {
      ...
      case RKEY_2:
      {
        ...
        Channel = TAPCOM_OpenChannel (TAPCOM_App_DemoServer, TAPCOM_DemoServer_ExecAdd, TAPCOM_DemoServer_ParameterVersion, (void*) &Parameter);
