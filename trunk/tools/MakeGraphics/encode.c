...
void split(node old)
{
	...
	n = avail;  avail = next[n];  childcount[n] = 0;
	t = prev[old];  prev[n] = t;  next[t] = n;
	t = next[old];  next[n] = t;  prev[t] = n;
	parent[n] = parent[old];
	level[n] = matchlen;
	position[n] = pos;
	makechild(n, text[matchpos + matchlen], old);
	makechild(n, text[pos + matchlen], pos);
}
...
static void get_next_match(void)
{
	...
	if (++pos == DICSIZ * 2) {
		...
		n = fread_crc(&text[DICSIZ + MAXMATCH], DICSIZ, infile); infile += n;
...
void encode(void)
{
	...
	remainder = fread_crc(&text[DICSIZ], DICSIZ + MAXMATCH, infile); infile += remainder; 
	// putc('.', stderr);
	matchlen = 0;
